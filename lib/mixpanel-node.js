...
var create_client = function (token, config) {
  ...
  metrics.send_request = function (endpoint, data, callback) {
    ...
    data.expire = moment().add('seconds', 60).unix()
    var sortedKeys = _.sortBy(_.keys(data), function (key) {
      return key;
    });

    var eventData = _.map(sortedKeys, function (key) {
      return key + '=' + data[key];
    });

    var eventDataEncoded = _.map(sortedKeys, function (key) {
      return key + '=' + encodeURIComponent(data[key]);
    });

    var eventDataString = eventDataEncoded.join("&");

    var sig = md5(eventData.join('') + config.api_secret);

    eventDataString += "&sig=" + sig;
    ...
    request_options.path = ['/api/2.0', endpoint, "?", eventDataString].join("");
    console.log(request_options.host + request_options.path);
  ...
  /**
   ...
   finished or an error occurs
   */
  metrics.segmentation = function (event, properties, callback) {
    ...
    var endpoint = '/segmentation'
    if (properties.inner && properties.outer) {
      endpoint += '/multiseg';
    }

    metrics.send_request(endpoint, properties, callback);
  };
